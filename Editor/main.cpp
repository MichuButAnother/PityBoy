...
#define PityBoy_Str_Version "0.20 Beta" // PityBoy name version
...
// Resolution can be change at the cost of some issues (?)
...
uint8_t palette[4][3] = {{230, 230, 0}, {204, 170, 0}, {128, 128, 0}, {64, 64, 0}};

int NOTEFREQ[] =                                                              // FREQ OF NOTES, stolen from my other project
...
sf::RenderWindow window(sf::VideoMode(W *scaling, H *scaling), string("PityBoy Editor ") + string(PityBoy_Str_Version)); // must use string() to avoid const char conversion etc issues
sf::Texture stexture;
sf::Sprite sscreen;
sf::Cursor cursor;
...
void pix(int x, int y, int color)
{
    if ((x + (y * 256)) > (W * H))
        return;
    pixels[((x + (y * 256)) * 4) + 0] = palette[color][0];
    pixels[((x + (y * 256)) * 4) + 1] = palette[color][1];
    pixels[((x + (y * 256)) * 4) + 2] = palette[color][2];
}
...
int getpix(int x, int y)
{
    if ((x + (y * 256)) > (W * H))
        return 0;
    if (palette[0][0] == pixels[((x + (y * 256)) * 4) + 0] && palette[0][1] == pixels[((x + (y * 256)) * 4) + 1] && palette[0][2] == pixels[((x + (y * 256)) * 4) + 2])
        return 0;
    if (palette[1][0] == pixels[((x + (y * 256)) * 4) + 0] && palette[1][1] == pixels[((x + (y * 256)) * 4) + 1] && palette[1][2] == pixels[((x + (y * 256)) * 4) + 2])
        return 1;
    if (palette[2][0] == pixels[((x + (y * 256)) * 4) + 0] && palette[2][1] == pixels[((x + (y * 256)) * 4) + 1] && palette[2][2] == pixels[((x + (y * 256)) * 4) + 2])
        return 2;
    if (palette[3][0] == pixels[((x + (y * 256)) * 4) + 0] && palette[3][1] == pixels[((x + (y * 256)) * 4) + 1] && palette[3][2] == pixels[((x + (y * 256)) * 4) + 2])
        return 3;
    return 0;
}
...
void clearscr(int color = 0)
{
    ...
    for (register int i = 0; i < W * H * 4; i += 4)
    {
        ...
        pixels[i] = palette[color][0];
        pixels[i + 1] = palette[color][1];
        pixels[i + 2] = palette[color][2];
        // pixels[i+3] = 255;
    }
}
...
int hex2dec(char in)
{ // converts F into 16 etc, its primitive function
    int c = 0;
    switch (in)
    {
    case '0':
...
void setfreq(int freq, int chan)
{
    if (chan == 0)
    {
        ch1_square.setFrequency(freq);
    }
    else if (chan == 1)
    {
        ch2_saw.setFrequency(freq);
    } // noise accepts only volume
}
...
// square wave
int ch1_tick(void *outputBuffer, void *inputBuffer, unsigned int nBufferFrames, double streamTime, RtAudioStreamStatus status, void *dataPointer)
{
    BlitSquare *square = (BlitSquare *)dataPointer;
    register StkFloat *samples = (StkFloat *)outputBuffer;
    if (ch1_volume > 0)
    {
        for (unsigned int i = 0; i < nBufferFrames; i++)
        {
            *samples++ = square->tick() * ch1_volume / 200;
            *samples++ = square->tick() * ch1_volume / 200;
        }
    }
    else
...
// Sawtooth
int ch2_tick(void *outputBuffer, void *inputBuffer, unsigned int nBufferFrames, double streamTime, RtAudioStreamStatus status, void *dataPointer)
{
    BlitSaw *sawtooth = (BlitSaw *)dataPointer;
    register StkFloat *samples = (StkFloat *)outputBuffer;
    if (ch2_volume > 0)
    {
        for (unsigned int i = 0; i < nBufferFrames; i++)
        {
            *samples++ = sawtooth->tick() * ch2_volume / 200;
            *samples++ = sawtooth->tick() * ch2_volume / 200;
        }
    }
    else
...
// Noise
int ch3_tick(void *outputBuffer, void *inputBuffer, unsigned int nBufferFrames, double streamTime, RtAudioStreamStatus status, void *dataPointer) // noise is louder than other channels
{
    Noise *noise = (Noise *)dataPointer;
    register StkFloat *samples = (StkFloat *)outputBuffer;
    if (ch3_volume > 0)
    {
        for (unsigned int i = 0; i < nBufferFrames; i++)
        {
            *samples++ = noise->tick() * ch3_volume / 200;
            *samples++ = noise->tick() * ch3_volume / 200;
        }
    }
    else
...
// options
bool skipstartanim = true; // DO NOT FORGET TO SET IT TO FALSE BEFORE RELEASE !!!!
bool smooth = false;
...
int main(int argc, char *argv[])
{
    ...
    // Custom palette checking //
    ifstream palettefile("palette.txt");
    uint8_t newpalette[4][3];
    if (palettefile.good())
    {
        string ss;
        int i = 1;
        ...
        int line = 1;
        bool success = false;
        int parsemode = -1; //-1 unknown, 0 rgb, 1 hex
        while (getline(palettefile, ss))
        {
            if (line == 1)
            {
                if (ss == "rgb" || ss == "RGB")
            ...
            else
            {
                if (parsemode == 0)
                {
                    ...
                    while (getline(istr, s, ' '))
                    {
                        ...
                        {
                        case 1:
                            newpalette[b - 1][0] = result;
                            break;
                        case 2:
                            newpalette[b - 1][1] = result;
                            break;
                        case 3:
                            newpalette[b - 1][2] = result;
                            break;
                        }
                        i++;
                        if (i > 3)
                ...
                if (parsemode == 1)
                {
                    ...
                    int r1 = hex2dec(ss[0 + c]);
                    int r2 = hex2dec(ss[1 + c]);
                    int g1 = hex2dec(ss[2 + c]);
                    int g2 = hex2dec(ss[3 + c]);
                    int b1 = hex2dec(ss[4 + c]);
                    int b2 = hex2dec(ss[5 + c]);
                    if (r1 == -1 || r2 == -2 || g1 == -1 || g2 == -2 || b1 == -1 || b2 == -2)
                        break;

                    newpalette[b - 1][0] = (r1 * 16) + r2;
                    newpalette[b - 1][1] = (g1 * 16) + g2;
                    newpalette[b - 1][2] = (b1 * 16) + b2;
        ...
        if (success)
        {
            memcpy(palette, newpalette, sizeof(palette) * 3 * 4);
        }
        else
        {
            cout << "palette.txt parsing error, using default palette. (Line " << line << ")" << endl;
        }
    }
    palettefile.close();
    ...
    for (register int i = 0; i < W * H * 4; i += 4)
    {
        ...
        pixels[i + 1] = palette[0][1];
        pixels[i + 2] = palette[0][2];
        pixels[i + 3] = 255;
    }
    ...
    // audio initialization and synth
    ...
    parameters.deviceId = dac_ch1.getDefaultOutputDevice(); // could be an issue later...
    parameters.nChannels = 2;
    RtAudioFormat format = (sizeof(StkFloat) == 8) ? RTAUDIO_FLOAT64 : RTAUDIO_FLOAT32 || RTAUDIO_MINIMIZE_LATENCY;
    unsigned int bufferFrames = RT_BUFFER_SIZE;

    try
    {
        dac_ch1.openStream(&parameters, NULL, format, (unsigned int)Stk::sampleRate(), &bufferFrames, &ch1_tick, (void *)&ch1_square);
        dac_ch2.openStream(&parameters, NULL, format, (unsigned int)Stk::sampleRate(), &bufferFrames, &ch2_tick, (void *)&ch2_saw);
        dac_ch3.openStream(&parameters, NULL, format, (unsigned int)Stk::sampleRate(), &bufferFrames, &ch3_tick, (void *)&ch3_noise);
        dac_ch1.startStream();
        dac_ch2.startStream();
        dac_ch3.startStream();
    }
    ...
    {
        error.printMessage();
        cout << "Audio initialization failed! PityBoy Editor will launch without audio." << endl;
    }
    ...
    // variables, probably could be done better
    ...
    memset(project.sfx, 0, sizeof(char) * 16 * 3 * 64);
    ...
    while (1)
    {
        while (1)
        {
            project.sfx[c][0][ci] = 16; // set volume to max without setting pitch
            c++;
            if (c > 15)
                break;
        }
    ...
    while (window.isOpen())
    {
        ...
        if (openedmenu == 1)
        {
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && window.hasFocus())
            {
                ...
                if (!laststate)
                {
                    ...
                    if (mousex > 70 && mousex < 186 && mousey > 60 && mousey < 71)
                        originbutton = 1;
                    if (mousex > 70 && mousex < 186 && mousey > 80 && mousey < 91)
                        originbutton = 1;
                }
                laststate = true;
            }
            else
            {
                if (laststate)
                {
                    ...
                    {
                        openedmenu = 2; // Project select menu
                        singlerun = true;
                        listselected = false;
                    }
                    if (mousex > 70 && mousex < 186 && mousey > 40 && mousey < 51 && originbutton == 1 && projectselected)
                    {
                        openedmenu = 3;         // Sprite editor
                        project.load_sprites(); // Reload sprites
                        singlerun = true;
                    }
                    if (mousex > 70 && mousex < 186 && mousey > 60 && mousey < 71 && originbutton == 1 && projectselected)
                    {
                        openedmenu = 4; // Map editor
                        singlerun = true;
                        mapsmenu = false;
                        project.load_maps(); // Reload maps
                    }
                    if (mousex > 70 && mousex < 186 && mousey > 80 && mousey < 91 && originbutton == 1 && projectselected)
                    {
                        openedmenu = 5; // SFX editor
                        singlerun = true;
                    }
                    originbutton = 0;
                    laststate = false;
            ...
            drawtext(string("PityBoy Editor ") + string(PityBoy_Str_Version), 1, 151, 1);
            ...
            if (mousex > 70 && mousex < 186 && mousey > 80 && mousey < 91 && projectselected)
                box(70, 80, 186, 91, 1, true);
            box(70, 80, 186, 91, color);
            drawtext("  SFX editor", 74, 82, color);
        }
        ...
        if (openedmenu == 2)
        {
            ...
            // THIS SCROLLBAR IS EXPERIMENTAL AND I AM NOT SURE HOW IT WORKS WITH LOT OF PROJECTS!!! and i know it gonna break //
        ...
        if (openedmenu == 4)
        {
            ...
            if (sf::Mouse::isButtonPressed(sf::Mouse::Middle) && window.hasFocus() && !mapdrag)
            {
                if (mapdm)
                {
                ...
                {
                    if (mousex >= 1 && mousex <= 254 && mousey >= 1 && mousey <= 148 && !mapsmenu)
                    {
                        mapdrag = true;
                        mapdragx = mousex;
                        mapdragy = mousey;
                    ...
                    while (1)
                    {
                        while (1)
                        {
                            ...
                            if (bx < 255 && bx > 0 && by > 0 && by < 149)
                            {
                                if (mapgrid && (i == 0 || b == 0))
                                {
                                    if (project.sprites[i][b][project.maps[vx][vy][mapindex]] != 0 && project.sprites[i][b][project.maps[vx][vy][mapindex]] != 2)
                                    {
                                        pix(bx, by, 2);
                                    }
                                    else
            ...
            if (mapdraw || mapdrag)
            {
                drawtext("X:" + to_string(cx) + " Y:" + to_string(cy) + " " + cp, 1, 151, 1);
            }
            else
            {
                switch (tooltip)
                ...
                case 7:
                    ...
                    break;
                default:
                    drawtext("Map editor " + cp, 1, 151, 1);
                    break;
                }
            }
        ...
        { // SFX editor
            ...
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && window.hasFocus())
            {
                ...
                if (!laststate)
                {
                    if (mousex >= 246 && mousex <= 255 && mousey >= 150 && mousey <= 159)
                        originbutton = 1;
                    if (mousex >= 105 && mousex <= 248 && mousey >= 8 && mousey <= 70)
                        originbutton = 100;
                    if (mousex >= 105 && mousex <= 248 && mousey >= 84 && mousey <= 142)
                        originbutton = 101;
                    if (mousex >= 5 && mousex <= 93 && mousey >= 8 && mousey <= 32)
                        originbutton = 102;
                    if (mousex >= 34 && mousex <= 41 && mousey >= 64 && mousey <= 71)
                        originbutton = 1;
                    if (mousex >= 44 && mousex <= 51 && mousey >= 64 && mousey <= 71)
                        originbutton = 1;
                    if (mousex >= 54 && mousex <= 68 && mousey >= 64 && mousey <= 71)
                        originbutton = 1;

                    // piano
                    if (mousex >= 20 && mousex <= 74 && mousey >= 120 && mousey <= 141)
                    {
                        pianoheld = 1;
                        pticks = -1;
                        psample = 0;
                ...
                if (pianoheld)
                {
                    if (mousex >= 20 && mousex <= 74 && mousey >= 120 && mousey <= 141)
                    {
                        if (mousex >= 24 && mousex <= 30 && mousey >= 120 && mousey <= 131)
                        {
                        }
                        else
                        {
                            ...
                            playnote = g + (poctave * 12);
                        }

                        setfreq(NOTEFREQ[playnote], project.sfx[0][2][sfxedit]);
                        setvol((project.sfx[psample][0][sfxedit] - 1) * 6.5, project.sfx[0][2][sfxedit]);
                        pticks++;

                        if (pticks > 90 - (pspeed * 10))
                        {
                            if (psample < 15)
                            {
                                psample++;
                                pticks = 0;
                                if (project.sfx[1][2][sfxedit] > 0)
                                    psample = 0; // repeat
                            }
                        }
                    }
                ...
                if (mousex >= 105 && mousex <= 248 && mousey >= 8 && mousey <= 70 && originbutton == 100)
                { // volume draw
                    int gx = (mousex - 105) / 9;
                    int gy = (mousey - 8) / 4;

                    project.sfx[gx][0][sfxedit] = 16 - gy;
                }

                if (mousex >= 105 && mousex <= 248 && mousey >= 84 && mousey <= 142 && originbutton == 101)
                { // pitch draw
                    int gx = (mousex - 105) / 9;
                    int gy = (mousey - 80) / 4;
                    project.sfx[gx][1][sfxedit] = 8 - gy;
                }

                if (mousex >= 5 && mousex <= 93 && mousey >= 8 && mousey <= 32 && originbutton == 102)
                { // SFX selector
                    int fx = (mousex - 5);
                    int fy = (mousey - 8);
                    if (fy > 4)
                        fy--; // stupid fix, but its working
                    if (fy > 8)
                        fy--;
                    if (fy > 12)
                        fy--;
            ...
            else
            {
                if (laststate)
                {
                    ...
                    if (mousex >= 34 && mousex <= 41 && mousey >= 64 && mousey <= 71 && originbutton == 1)
                    {
                        project.sfx[0][2][sfxedit] = 0;
                    }
                    if (mousex >= 44 && mousex <= 51 && mousey >= 64 && mousey <= 71 && originbutton == 1)
                    {
                        project.sfx[0][2][sfxedit] = 1;
                    }
                    if (mousex >= 54 && mousex <= 68 && mousey >= 64 && mousey <= 71 && originbutton == 1)
                    {
                        project.sfx[0][2][sfxedit] = 2;
                    }
            ...
            box(0, 150, 255, 159, 3, true);
            box(246, 150, 255, 159, 1, true);
            drawtext("SFX Editor - " + to_string(sfxedit), 1, 151, 1);
            if (mousex >= 246 && mousex <= 255 && mousey >= 150 && mousey <= 159)
                box(246, 150, 255, 159, 2, true);
            drawtext("<", 248, 151);
            ...
            if (pianoheld)
            {
                box(104 + (psample * 9), 0, 113 + (psample * 9), 149, 2, true);
            }

            // volume editor 16x16 (px 9x4)

            drawtext("Volume", 152, 0, 2);
            ...
            while (1)
            {
                ...
                while (1)
                {
                    ...
                    if (16 - project.sfx[dx][0][sfxedit] <= dy)
                    {
                        box(105 + (dx * 9), 8 + (dy * 4), 112 + (dx * 9), 10 + (dy * 4), 3, true);
                    }
                    else
                    {
                        box(105 + (dx * 9), 8 + (dy * 4), 112 + (dx * 9), 10 + (dy * 4), 1, true);
                    }
            ...
            while (1)
            {
                ...
                while (1)
                {
                    ...
                    if (8 - project.sfx[dx][1][sfxedit] == dy)
                    {
                        box(105 + (dx * 9), 80 + (dy * 4), 112 + (dx * 9), 82 + (dy * 4), 3, true);
                    }
                    else
                    {
                        if (dy == 8)
                        {
                            box(105 + (dx * 9), 80 + (dy * 4), 112 + (dx * 9), 82 + (dy * 4), 2, true);
                        }
                        else
                        {
                            box(105 + (dx * 9), 80 + (dy * 4), 112 + (dx * 9), 82 + (dy * 4), 1, true);
                        }
                    }
                    dy++;
                    if (dy >= 16)
            ...
            while (1)
            {
                ...
                while (1)
                {
                    ...
                    if (sfxedit == dx + (dy * 16))
                    {
                        box(5 + (dx * 5) + xoff, 8 + (dy * 6), 8 + (dx * 5) + xoff, 11 + (dy * 6), 1, true);
                    }
                    else
                    {
                        box(5 + (dx * 5) + xoff, 8 + (dy * 6), 8 + (dx * 5) + xoff, 11 + (dy * 6), 2, true);
                    }
                    if ((dx + 1) % 4 == 0 && dx != 0)
                        xoff = xoff + 2;
                    dx++;
            ...
            drawtext("SFX type(CH)", 2, 53);
            if (project.sfx[0][2][sfxedit] == 0)
            {
                box(34, 64, 41, 71, 2, true);
                drawtext("Square (1)", 8, 74, 2);
            }
            else
            {
                box(34, 64, 41, 71, 1, true);
                if (mousex >= 34 && mousex <= 41 && mousey >= 64 && mousey <= 71)
                    box(34, 64, 41, 71, 2, true);
            }
            drawicon(squarewave_icon, 34, 64, 1);
            if (project.sfx[0][2][sfxedit] == 1)
            {
                drawtext("Saw (2)", 8, 74, 2);
                box(34 + 10, 64, 34 + 17, 71, 2, true);
            }
            else
            {
                box(34 + 10, 64, 34 + 17, 71, 1, true);
                if (mousex >= 44 && mousex <= 51 && mousey >= 64 && mousey <= 71)
                    box(34 + 10, 64, 34 + 17, 71, 2, true);
            }
            drawicon(sawwave_icon, 44, 64, 1);
            if (project.sfx[0][2][sfxedit] == 2)
            {
                box(34 + 20, 64, 34 + 27, 71, 2, true);
                drawtext("Noise (3)", 8, 74, 2);
            }
            else
            {
                box(34 + 20, 64, 34 + 27, 71, 1, true);
                if (mousex >= 54 && mousex <= 68 && mousey >= 64 && mousey <= 71)
                    box(34 + 20, 64, 34 + 27, 71, 2, true);
            }
            drawicon(noise_icon, 54, 64, 1);

            // one octave piano

            drawtext("+\n4\n-", 80, 120);

            box(20, 132, 26, 141, 1, true);
            box(28, 132, 34, 141, 1, true);
            box(36, 132, 42, 141, 1, true);
            box(44, 132, 50, 141, 1, true);
            box(52, 132, 58, 141, 1, true);
            box(60, 132, 66, 141, 1, true);
            box(68, 132, 74, 141, 1, true);

            box(24, 120, 30, 131, 2, true);
            box(32, 120, 38, 131, 2, true);
            box(48, 120, 54, 131, 2, true);
            box(56, 120, 62, 131, 2, true);
            box(64, 120, 70, 131, 2, true);
        }
